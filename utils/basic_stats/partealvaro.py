...
fwds = players[players["main_position"].str.contains("FW|ST|CF|LW|RW|Winger|Forward", case=False, na=False)]
...
def top_player(df, metric, cols=None):
    ...
    return {c: (round(float(row[c]), 2) if isinstance(row[c], (float, np.floating)) else row[c]) for c in cols}
