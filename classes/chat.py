...
if USE_GEMINI:
    ...
    from settings import LM_STUDIO_API_BASE, LM_STUDIO_API_KEY, LM_STUDIO_CHAT_MODEL
else:
...
class PlayerChat(Chat):
    ...
    def handle_input(self, input, reasoning_effort=None, temperature=1, stream=False):
        if USE_GEMINI or USE_LM_STUDIO:
            super().handle_input(
                input, reasoning_effort=reasoning_effort, temperature=temperature, stream=stream
            )
            return
