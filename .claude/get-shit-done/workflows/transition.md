...
```bash
TRANSITION=$(node "D:/Twelve_Context_Eng/twelve-gpt-educational/.claude/get-shit-done/bin/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "D:/Twelve_Context_Eng/twelve-gpt-educational/.claude/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
if [ -n "$GSD_WORKSTREAM" ]; then
  WS_LIST=$(node "D:/Twelve_Context_Eng/twelve-gpt-educational/.claude/get-shit-done/bin/gsd-tools.cjs" workstream list --raw)
fi
```
...
```bash
node "D:/Twelve_Context_Eng/twelve-gpt-educational/.claude/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false
```
