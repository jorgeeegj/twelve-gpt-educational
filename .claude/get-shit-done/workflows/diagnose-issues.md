...
```bash
AGENT_SKILLS_DEBUGGER=$(node "D:/Twelve_Context_Eng/twelve-gpt-educational/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-debugger 2>/dev/null)
```
...
```bash
node "D:/Twelve_Context_Eng/twelve-gpt-educational/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs({phase_num}): add root causes from diagnosis" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
</step>
