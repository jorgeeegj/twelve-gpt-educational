...
from evals.judges.refuse_judge import judge_refuse
...
def _validate_fixture(entries: list[dict]) -> list[str]:
    ...
    for i, entry in enumerate(entries):
        ...
        if entry["expected_behavior"] not in _VALID_BEHAVIORS:
            errors.append(
                f"Entry {entry['id']}: invalid expected_behavior {entry['expected_behavior']}"
            )
    return errors
...
def _classify_failure(row: dict, expected_behavior: str) -> Optional[str]:
    ...
    if row["answer"].startswith("ERROR:"):
        return "error"
    # Raw-key check only applies to answerable questions — a refusal naming an
    # alternative stat may legitimately reference a column name in that context.
    if expected_behavior == "answerable" and not row.get("raw_key_passed", True):
        return "raw_key"
    if expected_behavior in ("refuse", "ambiguous_clarify") and row.get("refuse_passed") is False:
        return "refuse_expected_but_answered"
    if expected_behavior == "answerable" and row.get("refuse_passed") is True:
...
def _evaluate_entry(
    entry: dict, BasicStatsAgent, skip_judges: bool, judge_client=None, judge_model: str = ""
) -> dict:
    ...
    # Faithfulness — answerable questions with expected_values
    faith_passed: Optional[bool] = None
    faith_reason = "skipped"
    if (
        not skip_judges
        and expected_behavior == "answerable"
    ...
    # Refuse judge — refuse and ambiguous_clarify questions
    refuse_passed: Optional[bool] = None
    refuse_reason = "skipped"
    if (
        not skip_judges
        and expected_behavior in ("refuse", "ambiguous_clarify")
        and judge_client is not None
        and not final_answer.startswith("ERROR:")
    ):
        refuse_result = judge_refuse(
        ...
        refuse_passed = refuse_result.passed
        refuse_reason = refuse_result.reason
    ...
    row["failure_category"] = _classify_failure(row, expected_behavior)
    return row
...
def run(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(
                _evaluate_entry, e, BasicStatsAgent, skip_judges, judge_client, judge_model
            ): e["id"]
            for e in entries
        }
