...
def calulatexG(df, npxG):
    ...
    headers_model = smf.glm(
        formula="Goal ~ Distance + Angle", data=headers, family=sm.families.Binomial()
    ).fit()
    # non-headers
    nonheaders_model = smf.glm(
        formula="Goal ~ Distance + Angle", data=non_headers, family=sm.families.Binomial()
    ).fit()
    # assigning xG
    # headers
    b_head = headers_model.params
    xG = 1 / (
        1 + np.exp(b_head[0] + b_head[1] * headers["Distance"] + b_head[2] * headers["Angle"])
    )
    ...
    xG = 1 / (
        1
        + np.exp(
            b_nhead[0] + b_nhead[1] * non_headers["Distance"] + b_nhead[2] * non_headers["Angle"]
        )
    )
...
def FinalThird(df):
    ...
    rtp_player.rename(
        columns={"end_x": "final_third_receptions", "nextPlayerId": "playerId"}, inplace=True
    )
...
def smartPasses(df):
    ...
    # find accurate
    smart_passes_made = smart_passes.loc[
        smart_passes.apply(lambda x: {"id": 1801} in x.tags, axis=1)
    ]
...
def GoalsAssistsKeyPasses(df):
    ...
    data = g_player.merge(a_player, how="outer", on=["playerId"]).merge(
...
summary = (
    summary.merge(npxg, how="left", on=["playerId"])
...
fig, ax = baker.make_pizza(
    ...
    kwargs_slices=dict(
        facecolor="cornflowerblue", edgecolor="#000000", zorder=2, linewidth=1
...
# add subtitle
fig.text(0.515, 0.942, "Premier League | Season 2017-18", size=15, ha="center", color="#000000")
plt.show()
...
for i, row in minutes_per_game.iterrows():
    ...
    if str(player_id) not in possesion_dict.keys():
...
percentage_dict = {
    key: value["team_passes"] / value["all_passes"] if value["all_passes"] > 0 else 0
...
# percentiles
percentiles = [
    int(stats.percentileofscore(summary_adjusted[column], salah_adjusted[column].iloc[0]))
