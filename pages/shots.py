...
from classes.visual import DistributionPlot, ShotContributionPlot, ShotVisual
...
from utils.utils import create_chat, select_player
...
selected_match_name = st.sidebar.selectbox("Select a Match", options=id_to_match_name.values())
...
selected_player_minute = st.sidebar.selectbox(
    "Select a Shot (Player - Minute):", options=shots_df["player_minute"].unique()
)
...
visuals2 = ShotContributionPlot(
    df_contributions=df_contributions, df_shots=shots_df, metrics=metrics
)
visuals2.add_shots(shots_df, metrics, id_to_number=id_to_number)
visuals2.add_shot(
    contribution_df=df_contributions,
    shots_df=shots_df,
    shot_id=shot_id,
    metrics=metrics,
